...
def dam_text(
    ...
    text += (
        "📊 **مشخصات فنی**\n"
        f"📏 ارتفاع سد: {height} m\n"
        f"💧 ظرفیت ذخیره: {storage} MCM\n"
    ...
    if geodar_id != "ثبت نشده":

        text += (
            f"🆔 GeoDAR ID: {geodar_id}\n\n"
        )

    if grand_id != "ثبت نشده":
...
async def show_single_dam(
    ...
    # صفحه‌ای که این سد در آن قرار دارد
    page = index // DAMS_PER_PAGE
